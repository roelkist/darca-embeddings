...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-embeddings-venv
    export POETRY_HOME := /tmp/poetry-embeddings-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-embeddings-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/embeddings-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-embeddings-cache
endif
