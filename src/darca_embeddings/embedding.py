...
class EmbeddingResponseError(EmbeddingException):
    ...
    returns a non-200 status code, or the response is not in the expected format.
    """
...
class BaseEmbeddingClient(ABC):
    ...
    from text input, typically by interacting with an external model or service.
    """
    ...
    def get_embedding(self, text: str) -> list[float]:
        ...
        :type text: str
        :returns: A floating-point vector (list) representation of the input text.
        :rtype: list[float]
    ...
    def get_embeddings(self, texts: list[str]) -> list[list[float]]:
        ...
        By default, this method calls :meth:`get_embedding` for each string in ``texts``.
        Subclasses may override this method to provide more efficient batch processing.

        :param texts: A list of input strings to embed.
        :type texts: list[str]
        :returns: A list of embedding vectors, where each vector is a list of floats.
        :rtype: list[list[float]]
        :raises EmbeddingException: If something goes wrong while generating embeddings.
        """
...
class OpenAIEmbeddingClient(BaseEmbeddingClient):
    ...
    :type model: str
    :raises EmbeddingAPIKeyMissing: If the required API key environment variable is not found.
    """
    ...
    def get_embedding(self, text: str) -> list[float]:
        ...
        try:
            ...
            raise EmbeddingResponseError(
                message="Unexpected failure during OpenAI embedding response parsing.",
...
class EmbeddingClient:
    ...
    :param backend: The backend provider to use, e.g. ``openai`` or ``huggingface``.
    :type backend: str
    :param kwargs: Additional keyword arguments passed to the backend client's constructor.
    :raises EmbeddingException:
        - If the chosen backend is not implemented or unsupported.
    """

    def __init__(self, backend: str = "openai", **kwargs):
        """
        Initialize the EmbeddingClient with the given backend and model settings.
        ...
        if backend == "openai":
            ...
            raise EmbeddingException(
                message=f"Embedding backend '{backend}' is not implemented yet.",
